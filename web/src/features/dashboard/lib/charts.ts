...
export function processChartData(
  ...
  data.forEach((item) => {
    ...
    const timeKey = getChartBucketTimestamp(
      timestamp,
      timeGranularity,
  ...
  const fillTimePoints = (times: number[]) => {
    ...
    const lastTime = getChartBucketTimestamp(
      Math.max(...data.map((item) => Number(item.created_at) || 0)),
      timeGranularity,
      timezoneOffsetMinutes
    )
    const intervalSec = TIME_GRANULARITY_SECONDS[timeGranularity]
    const padded = Array.from(
      { length: MAX_TREND_POINTS },
      (_, i) => lastTime - (MAX_TREND_POINTS - 1 - i) * intervalSec
    )
    return padded
  }
  ...
  chartTimes.forEach((time) => {
    let timeData = sortedModels.map((model) => {
      ...
      return {
        Time: formatChartTime(time, timeGranularity, timezoneOffsetMinutes),
        Model: model,
        rawQuota,
        Usage: usage,
        TimeSum: 0,
