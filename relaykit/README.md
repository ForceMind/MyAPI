...
RelayKit 是 MyAPI 发行版中的独立 Go 模块，提供常用大模型文本协议的 DTO、请求转换、响应转换和流式事件转换。

它只负责协议层的数据建模与语义转换，不包含 HTTP 服务、上游请求发送、渠道调度、鉴权、计费或数据库逻辑。因此可以脱离 MyAPI 主模块，嵌入其他 Go 网关或代理服务。
...
- 作为独立 Go module 构建，不依赖 MyAPI 主模块、Gin、数据库或全局设置
...
```bash
go get github.com/ForceMind/MyAPI/relaykit@latest
```
...
RelayKit 是 MyAPI 项目的一部分，遵循项目根目录中的 [GNU Affero General Public License v3.0](../LICENSE)。
