...
MyAPI 面板鉴权采用短期 Access Token、HttpOnly Refresh Cookie 与服务端登录会话控制面的组合。面板请求不再依赖 Gin session，也不再要求旧版兼容请求头（`New-Api-User`）。
...
用户级模型成功请求限流仅在上游成功后记录，失败请求不占用成功额度。Redis 与内存实现都按每个用户计数 key 保存动态窗口；每个 key 使用自身过期时间，配置窗口变化不会续期其他 key。缩小 count 后会先清除窗口外记录，再按新上限判断仍在窗口内的记录。Enabled 状态不能配置为 0 分钟，窗口计算和容量受溢出及预分配保护。该机制仍是 check→execute→record 的近似合同：并发请求可能在记录前同时通过；如业务需要硬限额，必须另行设计 reservation/rollback 合同。滚动升级期间，旧节点写入的本地时间字符串和新节点写入的 UTC 字符串无法从格式上区分，可能在一个模型限流窗口内临时误放行或误拒绝；所有节点升级完成并经过一个完整窗口后会自然收敛，本次不切换 Key 或主动删除现有列表。

开放注册仍会受 Critical IP 限流保护，但分布式 IP 多账号攻击不能仅靠 IP 限流阻止。公网开放注册的部署应同时启用 Turnstile 和邮箱验证；更强的设备或多维风控需作为独立安全项目设计。

## PAT 调用契约

`User.AccessToken`（面板 PAT）继续支持 `Authorization: Bearer <pat>`，也兼容原有的单值 `Authorization: <pat>`。旧版 `New-Api-User` 请求头不再参与鉴权，外部脚本不需要再发送 Bearer 与用户 ID 双请求头。这是有意的调用契约简化；旧 PAT 本身无需重新生成。
...
- 用户级模型成功请求限流仅记录成功请求，且动态窗口按 key 自身过期；其 check→execute→record 并发语义仍可能短暂超发。UTC 时间戳在滚动升级期间存在一个窗口的混合格式过渡，期间可能临时误放行或误拒绝。
- 自建客户端应按新的 AuthBundle、`flow_token` 和 Security Proof 契约升级；PAT 客户端可直接移除旧版 `New-Api-User` 请求头。品牌迁移不会改变这些认证协议字段。
