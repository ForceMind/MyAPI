...
| S2 核心业务合同 | 进行中／已验收项见下表 | sol 实现与独立复审，terra 回归矩阵 | A01–A06/R1、B1、C01/C02/C03a/C04/C05/C06/C07/C08、D01–D10 已完成既有范围；C09-R3 已完成，C10 Codex 本机导入后端已完成本机定向/race/独立复审，待同提交 CI。B2/B3 任务持久化/恢复与 C03b 缓存恢复仍未完成。完整验证仍包括请求→预扣→上游→结算/退款→日志与 Provider 边界，relaykit 独立。 |
...
| S5 额度闭环与选定扩展 | 进行中／额度分析分层已完成本机验证 | sol 领域/安全，terra 常规实现 | 概览/渠道详情分层、三种速率方法、reset-aware ETA、有界折线和实际浏览器回归已完成本机范围，待同提交 CI/Docker；真实账户连续采样、通知、多 Key 身份和 Claude 组织用量仍按外部条件/决策登记，不混入平台账本。 |
...
**S2-C10 Codex 本机登录检测与导入（后端完成本机验收，UI 进行中）：** Full 原生仅从
...
发行与桌面合同。前端向导/i18n/视觉已在后续工作树完成本机验收，仍待自身同提交 CI；
真实账号导入不由合成测试代替。

**S5-Q1/Q2/Q3 额度分析分层（本机已验证，待同提交 CI）：** 保留既有快照 schema 和
...
陈旧倒计时、series key 与 sparkline 无障碍问题均已修复。未读取生产数据或真实凭据；
同提交 CI、Docker smoke 和真实账号连续采样仍待。
