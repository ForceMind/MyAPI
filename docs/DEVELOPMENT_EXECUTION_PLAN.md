...
> 2026-09-04 代码验收基线：`615fbbd`（CI `33800052236` 七项成功）；最新适用镜像证据仍为 `237c0da` 的 Docker smoke `33790513455`，Full/LAN 均成功。本文件管理完整路线与当前授权；历史证据保留在
...
| S2 核心业务合同 | 进行中／已验收项见下表 | sol 实现与独立复审，terra 回归矩阵 | A01–A06/R1、B1、C01/C02/C03a/C04/C05/C06/C07、D01–D03 已完成当前范围；D04 本地完成、待同提交 CI。B2/B3 任务持久化/恢复、C03b 缓存恢复及 D05–D10 仍未完成。完整验证仍包括请求→预扣→上游→结算/退款→日志与 Provider 边界，relaykit 独立。 |
...
| D04 Provider 响应/Vertex token | 本地已完成／待同提交 CI | 5/3 | SiliconFlow、Tencent、Vertex；合法/错误/malformed 响应，Vertex 固定安全错误、非空 token 与 HTTP/provider error 边界；不访问真实上游。 |
...
代码/纯 parser 测试确认。无页面变化，版本仍 0.1.1；待同提交 CI 后标 D04 完成。
