...
| S2 核心业务合同 | 进行中／已验收项见下表 | sol 实现与独立复审，terra 回归矩阵 | A01–A06/R1、B1、C01/C02/C03a/C04/C05 已完成当前范围；C06 的 CI race 补项待验证，B2/B3 任务持久化/恢复及 C03b 缓存恢复仍未完成。完整验证仍包括请求→预扣→上游→结算/退款→日志与 Provider 边界，relaykit 独立。 |
...
| S4 运行与恢复 | 进行中／S4-01 已完成；S4-02 未开始 | 工程/制品与独立 sol 审查；sol 数据与恢复 | Full/LAN 测试镜像均不推送；SQLite/MySQL/PostgreSQL 临时库旧结构升级、重复迁移、多连接竞争、备份恢复；登录/Key/权限/日志/额度探针；原生桌面构建、安装升级与第二设备 LAN。 |
...
| C06 | 待验证／测试隔离已修、本机 race 与独立复审通过；新 CI 步骤待跑 | 生产 logger 计数/轮转预约状态及轮询测试共享对象 | 两个渠道并发记录日志无 data race；保留日志格式与轮转、多渠道并发；测试使用独立快照/通知，不用串行化或 sleep 掩盖。 |
...
CI race 接线，本机与独立复审通过，待新提交 CI。未更改生产 C03b 策略，也不以此前
race 通过覆盖新失败。未将 S2-B2/B3、C03b、
S3–S7 或真实设备范围列为完成。
...
| 项目 | 当前事实 | 恢复/进入条件 |
| --- | --- | --- |
| GitHub CI | `7f1913e` / `33781560507` 七项 success；本批两库各七支付＋五快照实跑，原始日志已核对 | 历史假绿、统计回归及 PG JSON 失败保留；后续仍核对各自 SHA。 |
