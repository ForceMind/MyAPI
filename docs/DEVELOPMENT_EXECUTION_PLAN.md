...
> 2026-09-03 执行基线：`f8aa3d8`（CI `33761897224` 七项成功）。本文件管理完整路线与当前授权；历史证据保留在
...
| S2 核心业务合同 | 进行中／S2-A 与 C 系列已验收项见下表 | sol 实现与独立复审，terra 回归矩阵 | A01–A06/R1、C01/C02/C03a/C04/C05 已完成；S2-B 任务快照/持久化/恢复及 C03b 缓存恢复仍未完成，具体设计与重要决策单列。完整验证仍包括请求→预扣→上游→结算/退款→日志与 Provider 边界，relaykit 独立。 |
...
| S4 运行与恢复 | 进行中／S4-01 无发布镜像测试；恢复合同待设计 | 工程/制品与独立 sol 审查；sol 数据与恢复 | Full/LAN 测试镜像均不推送；SQLite/MySQL/PostgreSQL 临时库旧结构升级、重复迁移、多连接竞争、备份恢复；登录/Key/权限/日志/额度探针；原生桌面构建、安装升级与第二设备 LAN。 |
...
后续范围：S2-B 的 B1 冻结任务计费快照实现与复审已闭环，最终回归及 CI/实库待验证；响应前持久化与终态账务恢复仍未实施；S2-C 的已确证
...
| C06 | 待验证／本机定向 race 与独立复审通过 | 生产 logger 计数/轮转预约状态及轮询测试共享对象 | 两个渠道并发记录日志无 data race；保留日志格式与轮转、多渠道并发；测试使用独立快照/通知，不用串行化或 sleep 掩盖。 |
...
**B1 待验证／PG JSON 补项已修，待实库复验**：`e7fffc2` 的 MySQL 五项快照通过，PG 五项在 INSERT 报 SQLSTATE 22P02；原七项支付两库仍通过。两处 Valuer 改传 JSON 文本，本机驱动编码红绿测试、四包完整回归/vet/build 与独立复审通过，仍须新 CI 证明真实数据库。复用已有 TaskBillingContext，不另造快照，使完整的提交时模型/实际分组/
...
MySQL5.7/PG9.6 仍须在新提交 CI 真正执行后验收。
...
Node 合成 API 12 项、YAML/Bash 语法检查及独立复审通过。`540cf32` 的手动测试
`33766140801` 两种镜像均构建/启动成功，但实际表单就绪后构建标识检查失败，未予验收。
Rsbuild 产物证实间接读取 env 丢失注入值而回退为 0000/local；已修为直接属性读取，
仍要求新提交的镜像复验，不放宽 SHA 校验。此项只验证 linux/amd64、新安装与前端启动，不是三库升级恢复、
...
提交 SHA；Full/LAN 仍须在新提交的镜像里通过真实浏览器三处一致标识检查。

2026-09-04 最终整合根模块全量/vet/build、定向 race、relaykit 独立 build/test 已通过；
B1/C06/S4 的独立代码审查及文档复核闭环。当前只剩本批提交后的实库、镜像和发行包
证据，未将 S2-B2/B3、C03b、S3–S7 或真实设备范围列为完成。

## 待决策与外部条件

下一项 **S4-02 未开始／现有 API 的只读方案已定位**：先在无 Redis、非 batch、关闭
渠道内存缓存的全新 SQLite Full/LAN 镜像中创建合成普通用户、钱包、受限 Key 和普通
...
| 项目 | 当前事实 | 恢复/进入条件 |
| --- | --- | --- |
| GitHub CI | S2-C `451bee3` / `33760303619` 七个 job success，含真实 Redis 36 场景，原始输出已核对 | 旧 `33749764180` 假绿及 `6bae734` 的收尾统计回归记录保留；后续仍核对各自 SHA。 |
| Docker | 当前 Mac 未发现 Docker CLI 或 Docker.app；S4-01 正补 Full/LAN 全新 SQLite smoke | GitHub 临时镜像测试已授权；新 workflow 实跑后记录 SHA/edition/image ID，不能当作 GHCR digest 或三库/业务恢复证据。 |
