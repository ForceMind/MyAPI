...
后续范围：S2-B 的 B1 冻结任务计费快照正在实现；响应前持久化与终态账务恢复仍未实施；S2-C 的已确证
...
**B1 进行中**：复用已有 TaskBillingContext，不另造快照；使完整的提交时模型/实际分组/
...
Node 合成 API 单测、YAML/Bash 语法检查已通过；镜像构建与浏览器实跑尚待新提交的 GitHub
手动测试，不用单测代替。此项只验证 linux/amd64、新安装与前端启动，不是三库升级恢复、
