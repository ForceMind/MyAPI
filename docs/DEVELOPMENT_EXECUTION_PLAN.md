...
> 2026-09-03 执行基线：`2d705a7`（CI `33752536294` 六项成功）。本文件管理完整路线与当前授权；历史证据保留在
...
| S2 核心业务合同 | 进行中／S2-A 已验收 | sol 实现与独立复审，terra 回归矩阵 | A01–A06 及 R1 已完成；S2-B 任务快照/持久化/恢复、S2-C 数值/缓存/隐私仍待实施，具体设计与重要决策单列。完整验证仍包括请求→预扣→上游→结算/退款→日志与 Provider 边界，relaykit 独立。 |
...
后续范围：S2-B 冻结任务计费快照、响应前持久化与终态账务恢复仍未实施；S2-C 的已确证
数值/审计/缓存脚本/HTTP 隐私修复正在推进，状态见下表。
...
| ID | 状态 | 已确定写入边界 | 最低验收 |
| --- | --- | --- | --- |
| C01 | 待验证 | 文本结算 token 合计；`service/text_quota.go` 与回归 | `MaxInt+1` 不溢负后清零收费/退款；正常、零值、边界及真实结算/管理员审计。 |
| C02 | 待验证 | Kling 首次 clamp 经内部 TaskInfo 到结算日志；公共 AuditMap | 正常 ceil/倍率不变；极值、NaN/Inf、±1e309、缩小倍率、零差额仍保留首个审计；JSON 不丢 other，普通用户不见 admin_info。 |
| C03a | 待验证 | 四个 quota Lua 脚本、预扣 Go 参数边界、非额度缓存刷新参数索引 | 写前验证所有额度字段/结果，不留下半次 HINCRBY；非法操作不从预扣入口回退 DB 绕过；正常欠费/退款保留。metadata 刷新不创建缺 Quota hash。 |
| C03b | 未开始／待决策 | 批量模式缺损缓存恢复及高层增减的 cache/DB 一致性 | 明确故障时新预扣是否 fail-closed、冷缓存/已丢缓存如何区分、未落库增量如何恢复；不默默以旧 DB 水合覆盖余额。 |
| C04 | 待验证 | 私有视频控制器及真实路由回归 | HTTP/data URL 与 handler 错误为 private/no-store，上游 CDN 缓存头不覆盖；owner/非 owner/匿名与 SSRF 不回退。 |
| C05 | 待验证 | 四种支付控制器、必要的定向访问日志过滤及测试 | INFO/WARN/ERROR 和生产访问日志不输出原始正文/签名/query/客户资料；验签/ACK/幂等保持；Pancake 验证边界如实标注。 |
...
本机 miniredis 用于先复现和回归；新增 CI 真实 Redis 7 矩阵（36 场景）待实跑。它要求
...
| 项目 | 当前事实 | 恢复/进入条件 |
| --- | --- | --- |
| GitHub CI | S2-A-R1 `767b17b` / `33751536908` 六个 job success，实库中文日志与数量断言及原始输出已核对 | 旧 `33749764180` 假绿记录保留；后续仍检查各自 SHA 和原始证据，不借用旧绿灯。 |
