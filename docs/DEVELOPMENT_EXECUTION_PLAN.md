...
> 2026-09-04 当前代码收口基线：功能提交 `6f8f85b`（CI `33861244725` 八项成功）；同 SHA 的 Docker smoke `33862317233` 中 Full/LAN 均成功。历史 `2d6acab` / `33814136556` 与 `237c0da` / `33790513455` 保留为各自范围的证据。本文件管理完整路线与当前授权；历史证据保留在
...
通过。代码 `423846b` 的 [CI 33835254816](https://github.com/ForceMind/MyAPI/actions/runs/33835254816)
八个 job 全部成功；Frontend job 实际执行了生产构建、完整 Vitest、隔离 Playwright/
Chromium 图表与响应式回归并上传截图工件。发布版本仍为受保护的 `0.1.1`，页面运行时构建
标识由 commit SHA 区分本批。

**S2-C10 Codex 本机登录检测与导入（已完成当前确认范围）：** Full 原生仅从
服务进程自己的 `CODEX_HOME/auth.json` 或用户目录 `.codex/auth.json` 读取，不接受路径参数、
不写回原文件，状态和导入响应不含 token/绝对路径。自动导入要求 Root、实时 dashboard
session、精确同源、CriticalRateLimit、no-store 与 Passkey/2FA proof；LAN edition 和容器
在文件访问前短路，官方镜像另固定 `MYAPI_RUNTIME_ENV=container`。渠道创建与 abilities
同事务；所有 Codex refresh 在本机可取消 gate 与 `system_task_locks` 跨实例命名租约内
重新读取当前 key 后执行，最终 CAS 与缓存 generation 防止并发轮换和旧全量快照覆盖。
本机定向普通/race、五包 vet 和独立三轮审查无剩余 P0–P2；后端提交 `a7499a5` 的
[CI 33857902218](https://github.com/ForceMind/MyAPI/actions/runs/33857902218) 八个 job 全部
成功，包括根模块/relaykit、race、MySQL5.7/PostgreSQL9.6 既有数据库矩阵、Redis、前端、
发行与桌面合同。完整向导与额度整合提交 `6f8f85b` 的
[CI 33861244725](https://github.com/ForceMind/MyAPI/actions/runs/33861244725) 八个 job 全部成功，
同 SHA 的 Full/LAN Docker smoke `33862317233` 也已成功；真实账号导入不由合成测试代替。

**S5-Q1/Q2/Q3 额度分析分层（已完成当前确认范围）：** 保留既有快照 schema 和
原始区间计算，新增 latest interval、有效时间加权 observed window、连续时间 EWMA、
每分钟/小时速率、coverage 与 reset-aware ETA。概览固定单次 24h 查询、1h 分析窗口、
30m 半衰期、最多四系列和 48 点分段折线，不再展示高级控件或发起每系列历史请求；
渠道页保留完整范围/颗粒/指标/折线面积柱状，并增加算法、分析窗口和半衰期控制。
本机额度 Go/OpenAPI、8 文件 106 项 Vitest、typecheck、lint、生产构建和真实 Chrome
合成回归通过；独立 Sol 审查三轮发现的零余额、EWMA 切分、极值、跨 reset ETA、内存引用、
陈旧倒计时、series key 与 sparkline 无障碍问题均已修复。`6f8f85b` 的同提交 CI 与 Full/LAN
...
`/` 语义。`gofmt` 与 `git diff --check` 也已通过。本机未执行根模块全量、race、真实 Redis、SQLite/MySQL/PostgreSQL
矩阵、relaykit、前端/浏览器或 Docker；未访问真实 WebAuthn 设备/OAuth 上游/生产。无 schema、前端 UI、发布配置或
版本号变更，`VERSION` 仍为 0.1.1。独立复审当前 diff 无 P0/P1/P2；其唯一 P3 是未来可增加确定性并发观察以强化
“单 Store”回归保护，当前实现已静态核对为单 writer lock / 单 Store。

R4 代码 `c5cf662` 与审计 tip `b77ae6b` 的 [CI 33900966323](https://github.com/ForceMind/MyAPI/actions/runs/33900966323)
八个 job 全部成功：根模块与 relaykit vet/build/test、四组 race、临时 MySQL 5.7/PostgreSQL 9.6/Redis、前端
typecheck/test/build/browser、桌面和发行合同均已实际执行。该数据库 job 仍是既有 S1/S2-A/C02 回归，R4 未新增 schema
或专用三库 Passkey 场景，不将其写成新 Passkey 方言证明。[Docker smoke 33901685068](https://github.com/ForceMind/MyAPI/actions/runs/33901685068)
在同一 `b77ae6b` 上串行完成 Full/LAN：两者以 `push:false` 构建隔离镜像，使用新 SQLite、合成上游、真实登录和前端
冒烟，均成功且 runner 已清理容器；未登录 GHCR、发布镜像、创建 tag、部署或访问生产。
...
**B2-0 合同已冻结，B2/B3 尚未实现。** 负责人于 2026-09-05 确认以下恢复与账务边界；此记录
不代表已新增 schema、代码、测试、CI 或生产开关。现代 Task 的 `submission_unknown`（提交结果未知）
与已受理后轮询结果未知的 `outcome_unknown` 均不得自动重发或退款；只能由上游可验证的结果，或带审计
记录的人工处置结束。进入 `DISPATCHING` 后，v1 禁止一切可能已送达请求的重试和跨渠道 failover，包含
Provider 内重试。

客户端幂等范围固定为 `token + HTTP method + operation kind`：同一 scope、key 和请求指纹复用原
operation；同一 scope/key 但指纹不同必须返回 `409`。活动 operation 不自动过期，终态后保留 180 天。
adaptor 必须先持久化 operation，随后由统一边界返回 `202` 及稳定的 operation/task ID；每一个状态都
必须可查询，不能由 adaptor 先写成功响应。

主库账务事件是唯一权威账本；分库/ClickHouse 日志只是带稳定 `billing_event_id` 的至少一次投影，用户
查询、导出和统计必须据此去重，重投不得变成重复用量。首版只覆盖现代 Task，不扩展 Midjourney。C03b
完成前恢复功能 gate 保持关闭；启用前须排空并升级所有旧 writer/poller，不承诺开关后新旧 worker 混跑。
现有终态 CAS/系统任务租约将被复用；不能只延迟 HTTP 响应就宣称恢复完成，不能把批量队列“已入队”当作
事务落账，也不能批量推断历史终态记录应退款。三库加法迁移和实际启用仍须在后续实现/验证阶段分别证明，
生产切换仍需另行批准；C03b 的缓存恢复决定不由 B2/B3 替代。
