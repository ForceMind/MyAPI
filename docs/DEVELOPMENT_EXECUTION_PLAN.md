...
> 2026-09-03 执行基线：`6fabc98`。本文件管理完整路线与当前授权；历史证据保留在
> [完成度审计](COMPLETION_AUDIT.md)，产品定义见[主计划](MYAPI_MASTER_PLAN.md)。
> 当前已确认执行 **S0＋S1 及 S1-R1**；原六项已完成，R1 进行中。后续新行为、迁移策略和发布分别确认，不由持续目标扩大权限。
...
| S1 安全与一致性 | 进行中／已确认 | sol 实现；独立 sol 复审 | 原六项已完成；追加 R1 验证同进程配置保存/后台重载的发布顺序、双写交错与失败释放。R1 验证与独立复审通过后结束本阶段，不推导跨实例一致性。 |
...
复审追加项 **S1-R1（P2，进行中／已确认）**：两个配置写请求，以及后台重载取得旧
...
原六项已完成状态。R1 尚未通过验证与独立复审前，不得把 S1 总阶段标为已完成。
...
| 项目 | 当前事实 | 恢复/进入条件 |
| --- | --- | --- |
| GitHub CI | 基线 `6fabc98` 的 `33740901999` 五个 job 成功，含真实构建的额度浏览器 fixture 和截图 | 历史 Billing/runner 失败不再是当前阻塞；R1 必须检查新的 SHA，不能借用旧绿灯。 |
