...
> 2026-09-08 线上配置修复：Chat→Responses 已按 Codex 类型 57 覆盖所有模型，修复渠道 2 / codex-auto-review 漏配；没有发布 0.1.3，线上代码版本仍为 0.1.2。详情见[路由执行记录](CHANNEL_ROUTING_EXECUTION.md)。
...
> 2026-09-08 最新状态：开发基线已为 `main` / `f6536ca`，PR #1 已合并，CI 十项成功，原三个开发分支均已清理，WIP 归档标签保留。下文旧分支、未同步和未提交陈述为历史模板，不能用于回退或重建旧现场。当前用户批准的新增任务是[多渠道分配管理](CHANNEL_ROUTING_EXECUTION.md)，恢复时先核对工作树和该记录；当前源码候选版本为 `0.1.2`；验证结果见该记录，尚未部署或推送。
