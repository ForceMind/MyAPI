...
本提示词用于接管已有 B2 草稿，不使用旧 Mac 交接提示词，不重新要求 HEAD 必须停在旧 main。
...
- 交接时 HEAD 513ea6d6e863883aa3415a785a4d858bcf7210b0，仅为 B2-0 文档合同提交。
- 前一 R4 tip 83f24c5，功能 c5cf662、审计 b77ae6b；不要重做已有 R4。
- B2 尚未完成；B2-1 首轮模型与密钥修复受硬限制定向通过，后续独立复审/修复与实库CI状态见完整计划2.3和审计。
- GitHub CLI 曾确认登录，但 B2-0 推送未完成；此前自动审批以可能外发内部业务文档为由拒绝。
  不把登录状态当成推送成功，不绕过自动审批。本地工作与同步阻塞分别处理。

我明确授权你接管并继续修复完整计划第 2.3 节列出的当前 B2 任务文件。
请先核对清单、当前内容、Git差异和已记录的提交；第2.1节是修复前历史指纹，不得用它回退修复后的文件。
如果发现清单以外改动或来源不清，停止写入并报告，不自行回退。
此授权仅针对已知交接草稿，不是忽略任何脏工作树的通用许可。
...
不是本机环境事实。不要在这批脏工作树上先 switch main / pull。
...
既有 B2-0 提交 513ea6d6e863883aa3415a785a4d858bcf7210b0 可推送至
ForceMind/MyAPI 的 codex/b2-durable-submissions 分支，仍需通过工具自动审批，不绕过拒绝。
