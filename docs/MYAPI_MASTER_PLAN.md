...
- 管理员「系统信息」新增只读 Runtime build 标识，可复制构建 revision，用于确认实际运行镜像是否包含最新前端改动；更新测试镜像后的现场核对仍待完成。
...
- 渠道详情折线图、移动端降级视图和只读额度健康指标（代码初版已完成；概览额度变化支持前台自动刷新、provider plan type 和错误状态；真实设备审查与通知策略待完成）。
- 采集失败、重置和不支持状态。
