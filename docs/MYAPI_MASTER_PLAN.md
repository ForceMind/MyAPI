...
| 渠道额度历史 | 后端和前端初版已实现 | 普通渠道与 Codex OAuth 渠道均已有历史查询、折线图、失败状态、可选保留清理和只读健康指标；Codex 历史采样目前由查询触发，后台定时采样、通知、阈值持久化和去重仍待业务决策 |
| 账户额度变化聚合 | 初版已实现 | 概览和管理员渠道页均可查看每分钟变化及最大变化排序；依赖已有快照采样，后台定时采样、跨账户订阅账单同步和通知仍待后续迭代 |
...
| LAN Lite 桌面体验 | 安全基线已实现 | Electron 默认回环、单实例、持久会话密钥和显式 `--allow-lan` 私网绑定已补齐；安装设置、图形化 LAN 开关、防火墙提示和跨平台产物验证仍待完成 |
