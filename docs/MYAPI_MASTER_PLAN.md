...
| LAN Lite 桌面体验 | 安全基线已实现 | Electron 默认回环、单实例、持久会话密钥和显式 `--allow-lan` 私网绑定已补齐，安装文档与资源护栏已补齐；图形化 LAN 开关、防火墙提示和跨平台产物验证仍待完成 |
...
- 静态官网和发行版选择页（基础交互与静态资源校验 workflow 已完成，真实浏览器/移动视觉审查与发布 workflow 待完成）。
