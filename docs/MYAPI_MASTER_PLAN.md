...
- 响应/SSE 日志内存边界已加固。
- 概览中的账户额度变化面板同时展示每分钟变化和 Codex 账户 24 小时可用额度折线图；多组 Codex 渠道/窗口可选择查看。
- 本机测试副本已在一次明确授权下切换到当前构建的 `local/new-api:myapi-ff5feb8` 并通过健康检查；SQLite 迁移和旧镜像回滚也已在临时副本完成。其他生产环境仍需由部署方按 `docs/UPGRADE_REHEARSAL.md` 完成副本验证并明确批准后再操作。
