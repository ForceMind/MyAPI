...
| TokenHub 风格静态官网 | 交互与审阅制品已实现 | 形成独立产品叙事、安装入口、发行版选择、安全说明、基础菜单/主题交互、静态资源自动校验和 main 变更自动生成的确定性 artifact workflow；真实浏览器/移动视觉审查与绑定域名的独立发布仍待完成 |
| 渠道额度历史 | 后端和前端初版已实现 | 普通渠道与 Codex OAuth 渠道均已有历史查询、折线图、失败状态、可选保留清理和只读健康指标；普通渠道已有可选、有界后台采样，Codex 历史采样仍由查询触发；告警阈值已支持默认关闭、原子持久化和只读状态展示，通知/去重仍待业务决策 |
