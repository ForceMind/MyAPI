...
- 补齐真实手机浏览器验证；前端 `tsgo -b` 与 Vitest（当前 61 个文件、274 个测试）已在有界容器通过，实机视觉仍不可由自动化替代。额度概览面板会在管理员会话挂载时按 `user.id + session.sid` 最多刷新一次 `/api/user/self`，避免旧权限快照导致误隐藏；真实手机仍需验收。
