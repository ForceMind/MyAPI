...
> 基线日期：2026-08-30
>
> 当前源码基线：`2ab6cc2`（后续阶段性提交以 Git 历史为准）
...
| TokenHub 风格静态官网 | 初版已实现 | 形成独立产品叙事、安装入口、发行版选择和安全说明；真实浏览器/移动视觉审查与发布工作流仍待完成 |
| 渠道额度历史 | 后端和前端初版已实现 | 自动采集、历史查询、折线图、服务端聚合/时区和失败状态已有；自定义范围、预警和保留策略仍待完成 |
...
| LAN Lite 桌面体验 | 安全基线已实现 | Electron 默认 LAN/回环、单实例和持久会话密钥已补齐；安装设置、LAN 开关、防火墙提示和跨平台产物验证仍待完成 |
| GHCR 自动升级 | CLI 初版已实现 | 生产端显式拉取、健康检查、环境备份和失败回滚已有；签名验证、数据备份演练和人工审批仍待完成 |
...
- 桌面进程默认以 `MYAPI_EDITION=lan` 和 `MYAPI_BIND_ADDRESS=127.0.0.1` 启动，避免无提示暴露到局域网；开放局域网必须由后续明确的设置流程选择。
