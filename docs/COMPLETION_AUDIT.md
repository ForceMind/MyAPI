...
| 概览额度变化 | `account-quota-changes-panel.tsx`、`codex-account-quota-chart.tsx`、60 秒前台刷新、Codex 账户选择与可配置时间范围/颗粒度/指标/折线或面积图、稳定额度 0 值解释、错误/plan type/只读告警状态测试；`a2528a2` 的跨登录身份查询缓存隔离与认证刷新回归测试；系列按计划/单位/窗口隔离；后台采样默认开启并可在监控设置中调整 | 代码、前端类型检查与定向测试已验证 | 需要由具备 `channel.read` 的真实管理员在概览页面验收；真实手机视觉仍待完成 |
